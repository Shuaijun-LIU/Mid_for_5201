...
class WarehouseSimulator:
    ...
    def prepare_simulation_data(self, order_items, orders, policy_matrix):
        ...
        # OPTIMIZATION: Sample data for faster simulation
        # Sample top 1000 products by order volume
        product_order_counts = simulation_data['product_id'].value_counts()
        top_products = product_order_counts.head(1000).index.tolist()
        simulation_data = simulation_data[simulation_data['product_id'].isin(top_products)]
        
        # Sample every 3rd day to reduce simulation time
        ...
        sampled_dates = unique_dates[::3]  # Every 3rd day
        simulation_data = simulation_data[simulation_data['order_date'].isin(sampled_dates)]
        
        print(f"OPTIMIZATION: Sampled to {len(simulation_data):,} order items")
        print(f"OPTIMIZATION: Using {len(top_products):,} top products")
        print(f"OPTIMIZATION: Using {len(sampled_dates):,} sampled days")
    ...
    def simulate_warehouse_operations(self, simulation_data, storage_capacity, scenario_config):
        ...
        # OPTIMIZATION: Limit simulation to first 60 days for faster execution
        if simulation_days > 60:
            unique_dates = unique_dates[:60]
            simulation_days = 60
            print(f"OPTIMIZATION: Limiting simulation to {simulation_days} days")
        
        # initialize tracking variables
        daily_metrics = []
        product_inventory = storage_capacity[['product_id', 'total_storage_capacity']].copy()
        # REALISTIC: Start at 50% capacity to create more realistic constraints
        product_inventory['current_stock'] = product_inventory['total_storage_capacity'] * 0.5
        
        # staff capacity based on level - REDUCED for more realistic constraints
        staff_capacity = {
            'Low': 20,      # Reduced from 50
            'Medium': 40,   # Reduced from 100
            'High': 80      # Reduced from 200
        }
        daily_pick_capacity = staff_capacity[scenario_config['staff_level']]
        
        # OPTIMIZATION: Process orders in batches for better performance
        for day_idx, current_date in enumerate(unique_dates):
            ...
            # OPTIMIZATION: Process orders in batches
            if len(day_orders) > 0:
                # group orders by product for batch processing
                product_orders = day_orders.groupby('product_id').size().reset_index(name='order_count')
                
                for _, product_order in product_orders.iterrows():
                    product_id = product_order['product_id']
                    order_quantity = product_order['order_count']
                    ...
                    if current_stock >= order_quantity and daily_fulfilled < daily_pick_capacity:
                        ...
                        daily_fulfilled += order_quantity
                    else:
                        # add to backlog or count as stockout
                        if current_stock < order_quantity:
                            daily_stockouts += order_quantity
                        else:
                            daily_backlog += order_quantity
            
            # replenishment logic - REDUCED rates for more realistic simulation
            if scenario_config['replenishment_frequency'] == 'Daily':
                self.apply_replenishment(product_inventory, storage_capacity, 0.05)  # 5% daily replenishment
            elif scenario_config['replenishment_frequency'] == 'Weekly' and day_idx % 7 == 0:
                self.apply_replenishment(product_inventory, storage_capacity, 0.15)  # 15% weekly replenishment
            elif scenario_config['replenishment_frequency'] == 'Bi-weekly' and day_idx % 14 == 0:
                self.apply_replenishment(product_inventory, storage_capacity, 0.25)  # 25% bi-weekly replenishment
    ...
    def apply_predictive_replenishment(self, product_inventory, storage_capacity, day_idx):
        ...
        if day_idx % 7 == 0:  # weekly replenishment
            for idx, row in product_inventory.iterrows():
                ...
                # higher replenishment for growth products - REDUCED rates
                replenishment_rate = 0.2 if 'Growth' in str(row) else 0.1
                replenishment_amount = max_capacity * replenishment_rate
